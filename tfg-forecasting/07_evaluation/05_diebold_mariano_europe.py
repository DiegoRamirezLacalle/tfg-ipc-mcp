"""
05_diebold_mariano_europe.py — DM tests HICP Eurozona

Comparaciones:
  1. C0 vs C1_inst, C1_mcp, C1_full (por familia y subperiodo)
  2. C1_inst vs C1_mcp (cual senial aporta mas)
  3. C1_inst vs C1_full, C1_mcp vs C1_full
  4. Cada foundation C0 vs SARIMA Europe (referencia baseline)
...
def load_baseline_europe() -> dict[str, pd.DataFrame]:
    """Carga rolling_predictions_europe.parquet con naive y sarima."""
...
def align_errors(df1, df2, h, period_start=None, period_end=None):
...
def run_dm_pair(name1, name2, df1, df2, period_name="global",
...
def print_dm_table(results):
    print(f"\n{'Comparacion':<42} {'Periodo':<18} {'h':>3} "
          f"{'DM-stat':>9} {'p-val':>8} {'Ganador':>14} {'N':>5}")
    print("-" * 100)
    for r in results:
        ...
        for h in HORIZONS:
            ...
            if m["dm_stat"] is None:
                print(f"{label:<42} {r['period']:<18} {h:>3} {'—':>9} {'—':>8} "
                      f"{'insuf':>14} {m.get('n',0):>5}")
                continue
            sig  = "**" if m["p_value"] < 0.05 else ("*" if m["p_value"] < 0.10 else "")
            win  = m["better"]
            print(f"{label:<42} {r['period']:<18} {h:>3} {m['dm_stat']:>9.4f} "
                  f"{m['p_value']:>8.4f} {win:>14}{sig} {m['n']:>5}")


def main():
    print("=" * 65)
    print("DIEBOLD-MARIANO TESTS — HICP Eurozona")
    print("Metrica: MAE (power=1) | ** p<0.05 | * p<0.10")
    print("=" * 65)
    ...
    if not preds:
        print("[!] Sin predicciones disponibles."); return
    ...
    # ── 4. Foundation C0 vs SARIMA Europe (baseline de referencia)
    ...
    print("\nNota: DM<0 => model1 mejor  |  DM>0 => model2 mejor")
