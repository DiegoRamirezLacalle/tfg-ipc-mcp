"""
22_chronos2_C1_mcp_europe.py — Chronos-2 C1_mcp HICP Eurozona

Covariables exclusivamente semanticas BCE/GDELT (experimento MCP puro):
  bce_shock_score   (corr residuos C0 = +0.36) ***
  bce_tone_numeric  (corr residuos C0 = +0.26) ***
  bce_cumstance     (corr residuos C0 = -0.58) ***
  gdelt_tone_ma6    (corr residuos C0 = +0.46) ***
  signal_available  (indicador de disponibilidad)

NaN antes de 2015-01 rellenados con 0 (neutral).
"""
...
def prepare_input(df, origin):
    ...
    # Para el futuro: signal_available=1, resto neutral (0)
    future = {}
    for c in EXOG_COLS:
...
def main():
    ...
    print(f"Covariables MCP: {EXOG_COLS}")
    print("=" * 60)
    ...
    if c0p.exists():
        ...
        for h in HORIZONS:
            ...
            if c0m and c1m:
                print(f"  h={h}: C0={c0m:.4f}  C1_mcp={c1m:.4f}  delta={c1m-c0m:+.4f} ({(c1m-c0m)/c0m*100:+.1f}%)")
