...
def build_nixtla_df(
    ...
    hist_df = pd.DataFrame({
        "unique_id": SERIES_ID, "ds": ctx_y.index, "y": ctx_y.values,
    })
...
def main():
    ...
    parser.add_argument("--test-run", action="store_true",
                        help="Only 5 origins to validate pipeline")
    ...
    if df_preds.empty:
        logger.warning("[!] No predictions generated. Check API key and data.")
        return
