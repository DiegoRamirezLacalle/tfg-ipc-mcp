"""
11_timesfm_C1_inst.py — TimesFM C1 institutional: Ridge XReg con EPU Europe

Arquitectura: TimesFM base (contexto IPC completo 2002+) + Ridge correction.
Ridge ajustado sobre ventana completa 2002:origin (EPU Europe disponible desde 1987).
Covariables Ridge: epu_europe_ma3, epu_europe_log, epu_europe_lag1
"""
...
# EPU Europe disponible desde 2002 completo — no hace falta recortar
...
def load_model():
    ...
    tfm.compile(forecast_config=timesfm.ForecastConfig(max_context=512, max_horizon=MAX_H, per_core_batch_size=1))
    print("[timesfm] Cargado"); return tfm

def compute_xreg_correction(df, origin):
    """Ridge sobre cambio mensual del IPC, ventana 2002:origin con EPU Europe."""
...
def run_rolling(df, model):
    ...
    for origin in tqdm(origins, desc=f"{MODEL_NAME} rolling"):
        # Contexto IPC completo desde 2002 (no recortar a 2015+)
...
def main():
    ...
    df = load_data(); print(f"Datos: {len(df)} obs")
    model = load_model()
    ...
    if c0p.exists():
        c0 = json.load(open(c0p)).get("timesfm_C0",{})
        print(f"\n--- C0 vs {MODEL_NAME} ---")
        for h in HORIZONS:
