"""
07_timegpt_C1_energy_only.py — TimeGPT C1 solo con variables energeticas
...
Sin signal_available, sin bce_*, sin gdelt_*. Solo precios de energia.
Todas las columnas tienen datos reales desde 2002 (proxy Brent via WTI,
proxy TTF via Henry Hub). Sin NaN.

Contexto: IPC completo desde 2002 + 3 covariables energia completas.
"""
...
def build_nixtla_df(
    ...
    """IPC completo + 3 covariables energia completas (sin NaN)."""
...
def main():
    ...
    print(f"Origenes: {n_origins} ({'TEST' if args.test_run else 'COMPLETO'})")
    print(f"Covariables: {EXOG_COLS}")
    print(f"Coste estimado: {n_origins} llamadas API TimeGPT")
    print("=" * 60)
